...
- [ ] [SPEC-3] [code-app] `App.tsx`: render `<div className="shell"><div className="term" data-route={page}>…</div></div>`; add phosphor state (default `'amber'`) + crt state (default `'on'`); `useEffect` sets `document.documentElement.dataset.tui` and `.dataset.crt`. Remove the flex-column inline `style` wrapper. — `apps/code/src/App.tsx`
- [ ] [SPEC-4] [code-app] `Header.tsx`: render `.term-title` (`.wm` `▌MAESTRO`, `.sep`, version label, `.mid` session/context, `.right` online `.pip` + tier + model) and a `.tabs` row of `.tab` items with `.n` key numbers and `.active` on the current page. Remove inline color/layout styles. — `apps/code/src/components/Header.tsx`
...
- [ ] [TEST-3] App sets `data-tui="amber"` and `data-crt="on"` on `document.documentElement` after mount, and renders `.shell` + `.term`. — `apps/code/src/__tests__/App.theme.test.tsx`
- [ ] [TEST-4] Header renders `.term-title` with `.wm` brand and `.tabs` with the active tab carrying the `.active` class on the current page. — `apps/code/src/components/__tests__/Header.test.tsx`
