...
describe('maestro CLI (integration)', function() {
  ...
  it('maestro list should return workflows', () => {
    const out = cp.spawnSync('node', ['maestro-cli/index.js', 'list'], { encoding: 'utf8' });
  ...
  it('maestro execute commit-generator --mock should run', () => {
    const out = cp.spawnSync('node', ['maestro-cli/index.js', 'execute', 'commit-generator', '--mock'], { encoding: 'utf8' });
    assert.strictEqual(out.status, 0);
