...
import type { Session, Phase, ExecutionNode, LLMActivityEntry, ExecutionLogEntry, Artifact, CommandHistoryEntry } from '@maestro/tui/types';
import type { Block } from '@maestro/tui/types';
import type { Workspace } from '@maestro/tui/types';
import type { Project } from '@maestro/tui/types';
import type { LLMHealth, LLMModel, LLMStatus, ModelPerformance } from '@maestro/tui/types';
import type { IApiClient, HealthResponse } from '@maestro/tui/types';
...
const ago = (minutes: number): string => new Date(Date.now() - minutes * 60000).toISOString();
...
const makePhases = (): Phase[] => [
  { id: 'creation', name: 'Phase 1: Creation', status: 'done', result: { iterations: 3, fitness: 0.92, tokenCount: 42 } },
  { id: 'optimization', name: 'Phase 2: Optimization', status: 'done', result: { iterations: 8, fitness: 0.95, tokenCount: 156 } },
  { id: 'validation', name: 'Phase 3: Validation', status: 'running' },
  { id: 'publish', name: 'Phase 4: Publish', status: 'pending' },
];

const makeExecTree = (): ExecutionNode[] => [
  { id: 'load-config', name: 'Load Configuration', status: 'done', type: 'tool', children: [] },
  {
    id: 'improvement-loop', name: 'Improvement Loop', status: 'running', type: 'while',
    children: [
      ...
      {
        id: 'evaluate', name: 'Evaluate Results', status: 'running', type: 'validator',
        children: [
...
const makeLLMActivity = (): LLMActivityEntry[] => [
  {
    nodeId: 'run-training', time: timeStr(5), duration: 2.4, responseLength: 1280,
  ...
  {
    nodeId: 'evaluate', time: timeStr(4), duration: 1.1, responseLength: 256,
  ...
  {
    nodeId: 'generate-improvements', time: timeStr(3), duration: 3.2, responseLength: 2048,
  ...
  {
    nodeId: 'run-training', time: timeStr(1), duration: 2.8, responseLength: 1536,
...
const makeExecutionLog = (): ExecutionLogEntry[] => [
...
const makeDescriptorSession = (): Session => ({
  id: MOCK_SESSION_ID,
  ...
  variables: {
    _monitorDescriptor: { layout: 'descriptor', title: 'Gen-Commit Training' },
    _phases: makePhases(),
    _executionTree: makeExecTree(),
    _activeWorkflow: 'gen-commit-workflow',
    _activeBlock: { id: 'compute-fitness', name: 'Compute Fitness', type: 'tool', status: 'running' },
...
const makeIdleSession = (): Session => ({
  ...
  variables: { currentFitness: 0.72, model: 'Qwen2.5-Coder-1.5B' },
});

const makeExecSession = (): Session => ({
  ...
  variables: {
    ...
    _executionTree: [
      ...
      {
        id: 'test-models', name: 'Test Models', status: 'running', type: 'for-each',
        children: [
...
const MOCK_BLOCKS: Block[] = [
  {
    ...
    taskFitness: { score: 0.78, dimensions: { completion: 0.82, quality: 0.75, costEfficiency: 0.70, reliability: 0.85 } },
  ...
  },
  {
    ...
    taskFitness: { score: 0.72, dimensions: { completion: 0.78, quality: 0.70, costEfficiency: 0.65, reliability: 0.80 } },
  ...
  },
  {
    ...
    taskFitness: { score: 0.68, dimensions: { completion: 0.72, quality: 0.65, costEfficiency: 0.60, reliability: 0.75 } },
    sessionIds: [],
  },
  { id: 'llm-generate', name: 'LLM Generate', type: 'tool', version: '3.0.0', description: 'Core inference tool \u2014 calls LLM provider', author: 'system', fitness: null, isAtomic: true, children: [], fitnessDimensions: null, taskFitness: null, sessionIds: [] },
  { id: 'json-validator', name: 'JSON Validator', type: 'tool', version: '1.1.0', description: 'Validates JSON output against schema', author: 'system', fitness: 0.99, isAtomic: true, children: [], fitnessDimensions: { performance: 0.99, specialization: 0.98, composability: 1.0 }, taskFitness: null, sessionIds: [] },
  { id: 'fitness-evaluator', name: 'Fitness Evaluator', type: 'tool', version: '2.0.0', description: 'Computes fitness score from quality criteria', author: 'system', fitness: null, isAtomic: true, children: [], fitnessDimensions: null, taskFitness: null, sessionIds: [] },
  { id: 'file-writer', name: 'File Writer', type: 'tool', version: '1.0.0', description: 'Writes content to filesystem paths', author: 'system', fitness: null, isAtomic: true, children: [], fitnessDimensions: null, taskFitness: null, sessionIds: [] },
  { id: 'diff-parser', name: 'Diff Parser', type: 'tool', version: '1.0.0', description: 'Parses git diff into structured format', author: 'system', fitness: null, isAtomic: true, children: [], fitnessDimensions: null, taskFitness: null, sessionIds: [] },
  { id: 'metrics-reporter', name: 'Metrics Reporter', type: 'tool', version: '1.0.0', description: 'Formats and writes metrics reports', author: 'system', fitness: null, isAtomic: true, children: [], fitnessDimensions: null, taskFitness: null, sessionIds: [] },
];
...
const MOCK_PROJECTS: Project[] = [
  { id: 'proj-001', name: 'my-repo', rootPath: 'C:\\Projects\\my-repo', containerStatus: 'running', maestroInfo: { blocks: 3, artifacts: 5, metrics: 2, logs: 8 }, sessionIds: [MOCK_SESSION_ID, MOCK_SESSION_IDLE_ID] },
  { id: 'proj-002', name: 'api-service', rootPath: 'C:\\Projects\\api-service', containerStatus: 'idle', maestroInfo: { blocks: 1, artifacts: 2, metrics: 1, logs: 3 }, sessionIds: [MOCK_SESSION_EXEC_ID] },
  { id: 'proj-003', name: 'frontend-app', rootPath: 'C:\\Projects\\frontend-app', containerStatus: 'idle', maestroInfo: { blocks: 0, artifacts: 0, metrics: 0, logs: 0 }, sessionIds: [] },
];

// ── LLM data ────────────────────────────────────────────────────

const MOCK_LLM_HEALTH: LLMHealth = {
  status: 'healthy', model: 'SmolLM2-1.7B-Instruct', backend: 'vllm', device: 'cuda:0', gpuMemory: '2.1GB / 8.0GB',
};
...
const MOCK_LLM_STATUS: LLMStatus = {
  activeModel: 'SmolLM2-1.7B-Instruct', maxTokens: 512, temperature: 0.3,
  totalRequests: 47, avgLatency: 2.1, peakLatency: 4.8, errorCount: 0,
  tokensIn: 12400, tokensOut: 8200, throughput: 142, uptime: '2h 15m', load: 'idle',
};

const MOCK_MODEL_PERFORMANCE: Record<string, ModelPerformance> = {
  'SmolLM2-1.7B-Instruct': {
    bestFitness: 0.95, sessionCount: 3,
    taskFitness: [{ task: 'JSON generation', fitness: 0.95 }, { task: 'Commit message', fitness: 0.92 }, { task: 'Code review', fitness: 0.78 }],
    fitnessHistory: [0.65, 0.72, 0.78, 0.85, 0.88, 0.92, 0.95, 0.95],
  },
  'Qwen2.5-Coder-1.5B-Instruct': {
    bestFitness: 0.72, sessionCount: 1,
    taskFitness: [{ task: 'JSON generation', fitness: 0.72 }, { task: 'Commit message', fitness: 0.68 }],
    fitnessHistory: [0.50, 0.58, 0.65, 0.70, 0.72],
  },
  'SmolLM2-360M-Instruct': {
    bestFitness: 0.45, sessionCount: 1,
    taskFitness: [{ task: 'JSON generation', fitness: 0.45 }],
    fitnessHistory: [0.20, 0.30, 0.38, 0.42, 0.45],
  },
  'Phi-3-mini-4k-instruct': {
    bestFitness: null, sessionCount: 0, taskFitness: [], fitnessHistory: [],
  },
};
...
class MockApiClient implements IApiClient {
  ...
  async _delay(ms: number = 5): Promise<void> {
    return new Promise(r => setTimeout(r, ms));
  }

  async listSessions(): Promise<Session[]> {
    await this._delay();
    return [makeDescriptorSession(), makeExecSession(), makeIdleSession(), makeDoneSession()];
  }
  ...
  async get(path: string): Promise<unknown> {
    await this._delay();
    if (path === '/api/workspaces') return MOCK_WORKSPACES;
    return [];
  }

  async post(): Promise<Record<string, unknown>> { return {}; }
  async put(): Promise<Record<string, unknown>> { return {}; }
  async delete(): Promise<Record<string, unknown>> { return {}; }

  getApiUrl(): string { return this.baseUrl; }
}
