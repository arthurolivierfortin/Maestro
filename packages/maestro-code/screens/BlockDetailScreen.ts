// @ts-nocheck
/**
 * BlockDetailScreen — Block detail page for maestro-code.
 *
 * Shows block info, fitness dimensions, linked sessions, and actions.
 * 4 panels: INFO, FITNESS, SESSIONS, ACTIONS.
 *
 * Adapted from maestro-monitor's BlockDetail — uses @maestro/tui directly,
...
const FitnessContent = ({ block }) => {
  ...
  if (fitness != null) {
    ...
    elements.push(
      h(Box, { key: 'bf-title', flexDirection: 'row' }, bold('Block Fitness')),
      h(Box, { key: 'bf-score', flexDirection: 'row' },
        muted('  Score: '), T(col, `${pct}%`, { bold: true }),
      ),
      h(Box, { key: 'bf-bar', flexDirection: 'row' },
        muted('  '), T(col, progressBar(pct, 16)),
      ),
    );
  }
  ...
  if (taskFit) {
    ...
    elements.push(
      h(Box, { key: 'tf-title', flexDirection: 'row' },
        bold('Task Fitness'), h(Text, null, ' '), T(tCol, `${tPct}%`, { bold: true }),
      ),
    );
...
const ActionsContent = () => {
  return h(Box, { flexDirection: 'column', paddingLeft: 1 },
    h(Box, { key: 'act-v', flexDirection: 'row' },
      h(Text, null,
        h(Text, { color: theme.shortcut.bracket }, '['),
        h(Text, { color: theme.shortcut.key }, 'v'),
        h(Text, { color: theme.shortcut.bracket }, '] '),
        primary('View source JSON'),
      ),
    ),
  );
};

// ── BlockDetailScreen component ──────────────────────────────
...
const BlockDetailScreen = ({ blockId, apiClient, onNavigate, onBack, onQuit, height }: BlockDetailScreenProps) => {
  const [selectedIndex, setSelectedIndex] = useState(0);
  const [activePanel, setActivePanel] = useState<'sessions' | 'actions'>('sessions');

  const { data: block } = useApiData(
    useCallback(() =>
      apiClient?.getBlock?.(blockId) ||
      apiClient?._fetch?.('GET', `/api/blocks/${blockId}`).catch(() => null),
      [apiClient, blockId]),
    10000
  );
  ...
  const { data: allSessions } = useApiData(
    useCallback(() =>
      ...
      apiClient?._fetch?.('GET', '/api/sessions').catch(() => []),
      [apiClient]),
    5000
  );
  ...
  return h(Box, { flexDirection: 'column', width: '100%', flexGrow: 1 },
    ...
    h(Panel, { title: 'BLOCK', width: '100%' },
      h(Box, { flexDirection: 'column', paddingLeft: 1 },
        ...
        b.description
          ? h(Box, { flexDirection: 'row' }, muted(b.description))
          : null,
      ),
    ),
