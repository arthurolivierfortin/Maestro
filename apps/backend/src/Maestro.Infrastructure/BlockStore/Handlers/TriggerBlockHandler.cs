...
{
    public class TriggerBlockHandler : IBlockTypeHandler
    {
        public BlockDefinition? Load(string folderPath)
        {
            ...
            var blockType = root.TryGetProperty("blockType", out var btEl) ? btEl.GetString() : "trigger";

            var def = BlockDefinition.Create(id ?? Guid.NewGuid().ToString(), name ?? id ?? "block", blockType ?? "trigger");

            var dict = new Dictionary<string, object>();
            var conf = Path.Combine(folderPath, "trigger.json");
            if (File.Exists(conf)) dict["triggerConfig"] = JsonSerializer.Deserialize<object>(File.ReadAllText(conf)) ?? new object();
