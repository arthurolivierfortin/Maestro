import { app as c, BrowserWindow as C, ipcMain as l, dialog as M, shell as L, Menu as E } from "electron";
import _ from "electron-updater";
import i from "node:path";
import { fileURLToPath as R } from "node:url";
import h from "node:fs";
import { spawn as w } from "node:child_process";
const { autoUpdater: g } = _, k = i.dirname(R(import.meta.url));
...
function I() {
  ...
  return i.join(t, "backend", "src", "Maestro.Api");
}
function j() {
...
async function Q() {
  var s, d;
  const t = I(), r = i.join(t, "Maestro.Api.csproj");
  if (!h.existsSync(r))
...
function S() {
  e = new C({
    ...
    e == null || e.show();
  }), e.webContents.setWindowOpenHandler(({ url: t }) => (L.openExternal(t), { action: "deny" })), D ? (e.loadURL(D), e.webContents.openDevTools()) : e.loadFile(i.join(process.env.DIST, "index.html")), e.on("closed", () => {
    e = null;
  });
}
function A() {
...
F ? (c.on("second-instance", () => {
  ...
  O(), A(), S(), B(), process.env.AUTO_START_LLM !== "false" && (console.log("Auto-starting LLM-Provider..."), x().then((t) => {
