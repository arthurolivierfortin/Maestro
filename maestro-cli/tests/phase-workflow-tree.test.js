 ...
 *
 * Verifies:
 *  - Parent phase node is ALWAYS present after expand
 *  - Done phases get cloned exec tree children with 'done' status
 *  - Running phases get real exec tree children
 *  - Pending phases get cloned exec tree children with 'pending' status
 *  - Phase prefix prevents ID collisions across phases
 *  - Nested exec node expand/collapse works per-phase
 *  - Summary line appended for done phases
 *  - No duplicate IDs in flat list
 *  - _phaseStatus propagated to exec children for styling
 *  - Badges keep real status (not grayed out)
 *
 * Run: node tests/phase-workflow-tree.test.js
 */

// Inline the pure functions (no React/Ink dependency needed for data tests)
...
const flattenExecNode = (node, result, depth, parentId, expandedSet, phasePrefix, phaseStatus) => {
  ...
  result.push({
    id, depth, hasChildren, isExpanded, parentId,
...
const flattenPhaseWorkflow = (phases, executionTree, expandedSet) => {
  ...
  for (const phase of phases) {
    ...
    result.push({
      id: phaseId, depth: 0, hasChildren, isExpanded, parentId: null,
...
console.log('=== flattenPhaseWorkflow — expand done phase ===');
{
  ...
  // Children: 3 exec nodes + 1 summary = 4
  ...
  // Remaining phases still present
  ...
  // Total: 1 phase + 3 exec + 1 summary + 3 collapsed phases = 8
...
console.log('=== flattenPhaseWorkflow — expand running phase ===');
{
  ...
  // 3 collapsed phases + 1 expanded phase + 3 exec children = 7
  ...
  // Running phase gets REAL statuses (not forced)
...
console.log('=== flattenPhaseWorkflow — multiple phases expanded ===');
{
  ...
  // creation(1) + exec(3) + summary(1) + optimization(1) + validation(1) + publish(1) + exec(3) = 11
  assertEq(flat.length, 11, 'total nodes = 11');

  // Verify ALL phases are present (never disappear)
  const phaseNodes = flat.filter(n => n.data._type === 'phase');
...
console.log('=== Phase prefix prevents ID collisions ===');
{
  ...
  // Both phases have load-agent but different prefixes
...
console.log('=== Nested exec node expand within phase ===');
{
  ...
  // creation + load-agent + improvement-loop(expanded) + 5 children + finalize + summary + 3 phases
...
{
  // Simulate: start collapsed, toggle creation, verify, toggle again, verify
  let expandedSet = new Set();

  // Step 1: all collapsed
  let flat = flattenPhaseWorkflow(PHASES, EXEC_TREE, expandedSet);
  assertEq(flat.length, 4, 'step 1: 4 collapsed phases');

  // Step 2: expand creation (simulates toggle)
  ...
  // Step 3: collapse creation (simulates toggle again)
  expandedSet = new Set(expandedSet);
...
{
  // Done phase children get _phaseStatus = 'done'
  const expanded = new Set(['creation', 'publish']);
  const flat = flattenPhaseWorkflow(PHASES, EXEC_TREE, expanded);

  // Creation is done — exec children should have _phaseStatus = 'done'
  const creationExec = flat.filter(n => n.data._type === 'exec' && n.id.startsWith('creation/'));
  assert(creationExec.length > 0, 'creation has exec children');
  assert(creationExec.every(n => n.data._phaseStatus === 'done'), 'all creation exec children have _phaseStatus=done');

  // Publish is running — exec children should have _phaseStatus = 'running'
  const publishExec = flat.filter(n => n.data._type === 'exec' && n.id.startsWith('publish/'));
  assert(publishExec.length > 0, 'publish has exec children');
  assert(publishExec.every(n => n.data._phaseStatus === 'running'), 'all publish exec children have _phaseStatus=running');
}
...
console.log('=== _phaseStatus for pending phases ===');
{
  ...
  const futureExec = flat.filter(n => n.data._type === 'exec');
  assert(futureExec.length > 0, 'pending phase has exec children');
  assert(futureExec.every(n => n.data._phaseStatus === 'pending'), 'all pending exec children have _phaseStatus=pending');
}
...
console.log('=== _phaseStatus propagated to nested children ===');
{
  ...
  // Deeply nested children under done phase should also have _phaseStatus = 'done'
  const deepChildren = flat.filter(n => n.data._type === 'exec' && n.depth === 2);
  assert(deepChildren.length === 5, 'has 5 depth-2 exec nodes');
  assert(deepChildren.every(n => n.data._phaseStatus === 'done'), 'deeply nested children have _phaseStatus=done');
}
...
console.log('=== Phase nodes have no _phaseStatus (only exec nodes) ===');
{
  ...
  const phaseNodes = flat.filter(n => n.data._type === 'phase');
  assert(phaseNodes.every(n => n.data._phaseStatus === undefined), 'phase nodes have no _phaseStatus');
}

console.log('=== Exec children preserve original status in data.status ===');
{
  // Running phase: exec children keep their real statuses
  ...
  assertEq(loadAgent.data._phaseStatus, 'running', 'but _phaseStatus = running (parent phase)');
  ...
  assertEq(genImprov.data._phaseStatus, 'running', 'and _phaseStatus = running');

  // Done phase: exec children have forced status but real _phaseStatus
  ...
  assertEq(clonedGenImprov.data._phaseStatus, 'done', '_phaseStatus matches phase status');
}
...
console.log('');
if (failed === 0) {
  console.log(`phase-workflow-tree.test.js: ALL ${passed} tests passed`);
} else {
  console.error(`phase-workflow-tree.test.js: ${failed} FAILED, ${passed} passed`);
  process.exit(1);
}
