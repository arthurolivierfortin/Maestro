...
/**
 * Flattens an execution tree into FlatNode[] respecting expanded set.
 * Each FlatNode: { id, depth, hasChildren, isExpanded, parentId, label, data }
 */
const flattenExecutionTree = (nodes, expandedSet, depth = 0, parentId = null) => {
  const result = [];
 ...
 */
const autoExpandRunningPath = (nodes, expandedSet, parentChain = []) => {
  ...
  for (const node of nodes) {
    ...
    if (isActive) {
      ...
      // Expand this node too if it has children
      if (hasChildren) {
